/**
 * WordPress normalization — pure, and free of any Node dependency.
 *
 * Deliberately separate from `wordpress.ts`. The build-time adapter reads these
 * through the filesystem-backed fetch helpers, while the browser-side live
 * refresh imports the very same schema and normalizers directly. Sharing this
 * module is what guarantees a story refreshed in a reader's browser is
 * normalized identically to one baked in at build time — there is no second
 * implementation to drift.
 *
 * Nothing here touches `node:fs`, `node:crypto` or the network, so it is safe
 * in a client component.
 */

import { z } from 'zod';

import type { RssItem } from '@/lib/net/rss';
...
export function normalizeMany(raw: unknown[], options: WordPressAdapterOptions): MediaItem[] {
